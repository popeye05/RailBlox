...
export function AuthBoundary({children}:{children:ReactNode}){
 ...
 const submit=async(e:FormEvent)=>{e.preventDefault();if(!auth)return;setBusy(true);setError('');setNotice('');try{
  ...
  const result=recovery?await auth.auth.updateUser({password}):register?await auth.auth.signUp({email,password,options:{emailRedirectTo:window.location.origin+'/queue',data:{full_name:name.trim(),...(config?.collect_date_of_birth?{date_of_birth:dob}:{}),requested_role:role}}}):await auth.auth.signInWithPassword({email,password});
 ...
 if(!identity||recovery||register)return <main className="auth-page"><div className="auth-intro"><div className="auth-logo"><img src={logo} alt="RailBLOX"/></div><p className="section-overline">Railway planning & coordination</p><h1>Every window.<br/>Every worksite.</h1><p>A shared workspace for maintenance priorities, evidence and officer-reviewed block planning.</p><div className="auth-division">Authorized access · {config?.division||'Connecting'}</div></div>
  <form className="auth-card" onSubmit={submit}><p className="section-overline">DIVISION WORKSPACE</p><h2>{recovery?'Set your password':register?'Create an account':'Sign in'}</h2><p>{register?'An administrator must approve your requested role before you can access this division.':'Use the account provided by your division administrator.'}</p>
   {!recovery&&<label>Work email<input type="email" autoComplete="username" required value={email} onChange={e=>setEmail(e.target.value)}/></label>}
